...
$em_first = $user['firstname'];
$em_last = $user['lastname'];
...
<div class='messages-headline'>
	<h4 id='reciept'>$em_first $em_last</h4>
...
$first = "SELECT * FROM messages WHERE sender = '$freelancer' OR recipient = '$freelancer' AND sender='$employer' OR recipient = '$employer' ORDER BY DATE(created) ASC ";
$first_res = mysqli_query($con, $first);
while ($row = mysqli_fetch_assoc($first_res)) {
    $mid = $row['id'];
    $date = $row['created'];

    $send = "SELECT * FROM messages WHERE sender = '$freelancer' AND recipient = '$employer' AND id = '$mid' LIMIT 1 ";
    ...
    $recieve = "SELECT * FROM messages WHERE sender = '$employer' AND recipient = '$freelancer' AND id = '$mid' LIMIT 1 ";
...
$final = " UPDATE messages SET `status_flc` = 'old' WHERE sender = '$freelancer' OR recipient = '$freelancer' AND recipient = '$employer' OR sender = '$employer' ";
...
<div class='message-reply'>
	...
	<button onclick='sendMessage($freelancer, $employer)' class='button ripple-effect'>Send</button>
</div>
";