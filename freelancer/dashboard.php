			...
			<div class="fun-facts-container">
				<div class="fun-fact" data-fun-fact-color="#36bd78">
					<div class="fun-fact-text">
						...
						<h4><?php
								$tasks = "SELECT * FROM mytask WHERE freelancer_id = '$id' "; 
				...
				<div class="fun-fact" data-fun-fact-color="#efa80f">
					...
					<div class="fun-fact-text">
						<span>Balance</span>
						<h4><?php 
						$sql = "SELECT * FROM wallet WHERE freelancer_id = '$id' LIMIT 1";
						$result = mysqli_query($con, $sql);
						$user = mysqli_fetch_assoc($result);
						echo $user['balance']; ?></h4>
					</div>
				...
				<div class="fun-fact" data-fun-fact-color="#2a41e6">
					<div class="fun-fact-text">
						...
						<h4><?php
						$da = date('F');
						$count = "SELECT * FROM profile_view WHERE user_id = '$id' AND MONTHNAME(created) = '$da' ";
				...
				<div class="col-xl-6">
					<div class="dashboard-box">
						...
						<div class="content">
							<ul class="dashboard-box-list">
							<?php 
							$sql = "SELECT * FROM orders WHERE freelancer_id = '$id' ORDER BY id DESC";
							$rs_result = mysqli_query ($con, $sql); //run the query 
							while ($row = mysqli_fetch_assoc($rs_result)) {
								$oid = $row['id'];
		...
		?>
			...
			// Information about the dataset
	   		datasets: [{
				...
				data: [
					<?php
					$count = "SELECT * FROM profile_view WHERE user_id = '$id' AND MONTHNAME(created) = '$months[0]' ";
					echo mysqli_num_rows(mysqli_query($con, $count));
					?>,
					<?php
					$count = "SELECT * FROM profile_view WHERE user_id = '$id' AND MONTHNAME(created) = '$months[1]' ";
					echo mysqli_num_rows(mysqli_query($con, $count));
					?>,
					<?php
					$count = "SELECT * FROM profile_view WHERE user_id = '$id' AND MONTHNAME(created) = '$months[2]' ";
					echo mysqli_num_rows(mysqli_query($con, $count));
					?>,
					<?php
					$count = "SELECT * FROM profile_view WHERE user_id = '$id' AND MONTHNAME(created) = '$months[3]' ";
					echo mysqli_num_rows(mysqli_query($con, $count));
					?>,
					<?php
					$count = "SELECT * FROM profile_view WHERE user_id = '$id' AND MONTHNAME(created) = '$months[4]' ";
					echo mysqli_num_rows(mysqli_query($con, $count));
					?>,
					<?php
					$count = "SELECT * FROM profile_view WHERE user_id = '$id' AND MONTHNAME(created) = '$months[5]' ";
					echo mysqli_num_rows(mysqli_query($con, $count));
					?>],
				pointRadius: 5,
				pointHoverRadius:5,
...
<script>
	$('#check_active_plan').click(function() {
			$.ajax({
        		...
        		type:"post",
				data: {
					'freelancer_id' : <?php echo $_SESSION['user_id']; ?>,
				},
    			success: function(response) {
      			if (response == 'NO_ACTIVE_PLAN') {
					...
					Snackbar.show({
						text: '<p style="text-align: center;">Your plan is still active.<br>Check your email for more details.</p>',
						duration: 10000,
						textColor: '#fff',
						backgroundColor: '#383838'
					});
