				...
				<div class="col-xl-12">
					<div class="dashboard-box margin-top-0">
						...
						<div class="content">
							...
							<?php 
							$sql = "SELECT * FROM tasks WHERE user_id = '$id' AND bid_status = 'inactive' ORDER BY id DESC";
							...
							while ($row = mysqli_fetch_assoc($rs_result)) {
								...
								$task = "SELECT * FROM mytask WHERE employer_id = '$id' AND task_id = '$tid' LIMIT 1";
								$taskres = mysqli_query($con, $task);
								$taskget = mysqli_fetch_assoc($taskres);
								$budget = $taskget['budget'];
								$delivery = $taskget['delivery'];
								$mtid = $taskget['id'];

								$delivery = date('D d M Y', strtotime($delivery));

								$as;
								if ($taskget['finished'] != null) {
									$as = "
									...
									<ul class='dashboard-task-info'>
										...
										<li><a href='jobscomplete.php?mytask_id=$mtid' class='button ripple-effect'>Job's Complete</a></li>
									</ul>";
								}
